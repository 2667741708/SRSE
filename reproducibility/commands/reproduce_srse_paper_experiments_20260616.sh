...
# The original c201 launchers are kept in this directory for provenance; this
# file rewrites the main commands with package-local paths and overrideable
# runtime variables.
#
# Example:
...
Runtime overrides:
  PROJECT_ROOT, CODE_ROOT, PY, DATA_ROOT, CROWD_ROOT, OUT, GPU_ID,
  MAX_RUN_EPOCHS, MAIN_SCRIPT, ABLATION_SCRIPT, PSS_SCRIPT, AGG_CROWD_SCRIPT.
USAGE
}
...
max_run_args=()
if [[ -n "${MAX_RUN_EPOCHS}" && "${MAX_RUN_EPOCHS}" != "0" ]]; then
  max_run_args=(--max_run_epochs "${MAX_RUN_EPOCHS}")
fi
