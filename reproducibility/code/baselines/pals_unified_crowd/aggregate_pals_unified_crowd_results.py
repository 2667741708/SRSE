...
def infer_meta(path: Path):
    ...
    if not name:
        return None
    m = re.match(r"(?P<dataset>[A-Za-z]+)_lpi(?P<lpi>\d+)_slice(?P<slice>\d+)_seed(?P<seed>\d+)_unified_pals_", name)
    if not m:
        return None
    return {
...
def write_outputs(rows, summary, out_dir: Path, tag: str):
    out_dir.mkdir(parents=True, exist_ok=True)
    detail_csv = out_dir / f"pals_unified_crowd_{tag}_detail.csv"
    summary_csv = out_dir / f"pals_unified_crowd_{tag}_summary.csv"
    ...
    md = out_dir / f"pals_unified_crowd_{tag}_summary.md"
    title_tag = tag.upper() if tag.lower().startswith("lpi") else tag
    lines = [f"# PALS Unified Crowd {title_tag} Summary", ""]
...
def main():
    ...
    if args.write_summary:
        write_outputs(rows, summary, args.write_summary, args.tag)
    for row in summary:
        print(row)
