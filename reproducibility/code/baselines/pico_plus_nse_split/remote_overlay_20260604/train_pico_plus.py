...
def main_worker(gpu, ngpus_per_node, args):
    ...
    best_acc = 0
    if args.resume:
        result_log = os.path.join(args.exp_dir, 'result.log')
        if os.path.isfile(result_log):
            try:
                with open(result_log, 'r') as f:
                    for line in f:
                        vals = [float(x) for x in re.findall(r'(?:Acc|Best Acc) ([0-9]+(?:\.[0-9]+)?)', line)]
                        if vals:
                            best_acc = max(best_acc, max(vals))
                print('=> restored best acc from result.log: {}'.format(best_acc))
            except Exception as exc:
                print('=> could not restore best acc from result.log: {}'.format(exc))
