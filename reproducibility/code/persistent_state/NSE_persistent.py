...
def parse_args():
    ...
    parser.add_argument('--ablate_no_cr', action='store_true',
                        help='[AuditAblation] Disable active-set weak/strong CR; keep only the strong-view supervised branch, with MixUp preserved when enabled.')
...
# --- 修改部分开始 ---
    ...
    parser.add_argument('--fredis_full_theta', type=float, default=1e-6,
                        help='[FREDIS-full] Initial refinement threshold theta from the official FREDIS update.')
    parser.add_argument('--fredis_full_inc', type=float, default=1e-6,
                        help='[FREDIS-full] Theta increment when a round changes fewer than change_size labels.')
    parser.add_argument('--fredis_full_delta', type=float, default=1.0,
                        help='[FREDIS-full] Initial disambiguation threshold delta from the official FREDIS update.')
    parser.add_argument('--fredis_full_dec', type=float, default=1e-6,
                        help='[FREDIS-full] Delta decrement used by the official FREDIS update.')
    parser.add_argument('--fredis_full_times', type=float, default=2.0,
                        help='[FREDIS-full] Require disambiguation changes to be at least times times refinement changes.')
    parser.add_argument('--fredis_full_change_size', type=int, default=500,
                        help='[FREDIS-full] Randomly sample at most this many refinement labels per update round.')
    parser.add_argument('--fredis_full_update_interval', type=int, default=20,
                        help='[FREDIS-full] Apply one source-edit round every K epochs, matching FREDIS update_interval.')
    parser.add_argument('--fredis_full_theta_end', type=float, default=0.9,
                        help='[FREDIS-full] Upper guard for theta scheduling.')
    parser.add_argument('--fredis_full_delta_end', type=float, default=0.1,
                        help='[FREDIS-full] Lower guard for delta scheduling.')
    ...
    parser.add_argument('--irnet_full_num_views', type=int, default=3,
                        help='[IRNet-full] Number of stochastic classifier views required for correction consistency.')
    parser.add_argument('--irnet_full_threshold_start', type=float, default=0.008,
                        help='[IRNet-full] Start value of the non-candidate-over-candidate score margin.')
    parser.add_argument('--irnet_full_threshold_end', type=float, default=0.008,
                        help='[IRNet-full] End value of the non-candidate-over-candidate score margin.')
    parser.add_argument('--irnet_full_correct_duration', type=int, default=2000,
                        help='[IRNet-full] Number of epochs after source_update_start_epoch during which correction is active.')
    parser.add_argument('--irnet_full_correct_update', type=str, default='case3',
                        choices=['none', 'case1', 'case2', 'case3'],
                        help='[IRNet-full] Confidence update case after modifying candidate membership.')
    parser.add_argument('--irnet_full_correct_deletion', action='store_true',
                        help='[IRNet-full] Also delete the lowest-scoring current candidate label for corrected samples.')
