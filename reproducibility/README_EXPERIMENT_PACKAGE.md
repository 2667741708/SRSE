...
- `reproducibility/code/data/read.md` and
  `reproducibility/DATASETS_README.md`: dataset download and expected layout
  notes.
- `reproducibility/code/utils/*.py`: shared model, augmentation, loss,
  topology, and training utilities.
...
For a short epoch-level validation, keep the original `--epochs` setting and
cap only the executed loop:
...
log. The current SRSE warmup compatibility setting is
