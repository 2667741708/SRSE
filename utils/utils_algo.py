...
def reliable_pseudolabel_selection_KNN_pseudolabel(args, device, trainloader, features, epoch, model_preds=None):
    ...
    # --- 第一次标签传播 ---
    knn_soft_labels_intermediate = soft_labels.expand(N, -1, -1)
    score_intermediate = torch.sum(
        torch.mul(torch.gather(knn_soft_labels_intermediate, 1, knn_indices), weights.view(N, -1, 1)), 1
    )
    pseudo_labels_intermediate = torch.max(score_intermediate, -1)[1]
    soft_labels_for_propagation = torch.zeros((N, args.num_classes)).scatter_(1, pseudo_labels_intermediate.view(-1, 1), 1)

    # --- 第二次标签传播 (生成最终的后验概率) ---
    knn_soft_labels_final = soft_labels_for_propagation.expand(N, -1, -1)
    score_final = torch.sum(
        torch.mul(torch.gather(knn_soft_labels_final, 1, knn_indices), weights.view(N, -1, 1)), 1
    ) 

    # [捕获] 这是用于共识的最终 KNN 伪标签
    final_knn_pseudo_labels = torch.max(score_final, -1)[1]
    
    # [计算] 这是用于不确定性筛选的后验概率
    soft_labels_posterior = score_final / score_final.sum(1).unsqueeze(-1)
    
    # --- 基于不确定性的样本筛选 ---
    prob_temp = soft_labels_posterior.clone()
    prob_temp.clamp_(min=1e-2, max=1-1e-2) # 使用 clamp 更简洁
    discrepancy_measure = -torch.log(prob_temp)

    # ... (计算 num_samples2select_class 的代码保持不变) ...
    num_clean_per_class = torch.zeros(args.num_classes)
    agreement_temp_mask = torch.zeros((N, args.num_classes))
    agreement_temp_mask[range(N), final_knn_pseudo_labels] = (labels[range(N), final_knn_pseudo_labels] == 1.0).float()
    for i in range(args.num_classes):
        num_clean_per_class[i] = torch.sum(agreement_temp_mask[:,i])
    ...
    print("Samples to select per class (target):", num_samples2select_class.item())

    # =========================================================================
    # [核心修复] 直接构建选择掩码和伪标签，避免使用 inf 和 torch.min
    # =========================================================================
    
    selected_mask = torch.zeros(N, dtype=torch.bool)
    final_selected_labels = torch.zeros(N, dtype=torch.long) # 使用安全的 0 作为默认标签

    for i in range(args.num_classes):
        # 1. 找到原始标签中包含类别 i 的所有候选样本
        candidate_indices_mask = (labels[:, i] == 1.0)
        if not candidate_indices_mask.any():
            continue
        
        candidate_indices = candidate_indices_mask.nonzero().squeeze()
        if candidate_indices.numel() == 1: # 处理只有一个候选样本的边缘情况
            candidate_indices = candidate_indices.unsqueeze(0)

        # 2. 获取这些候选样本对类别 i 的不确定性分数
        discrepancy_class = discrepancy_measure[candidate_indices, i]

        # 3. 确定要选择多少个样本
        num_to_select = min(int(num_samples2select_class.item()), len(candidate_indices))
        if num_to_select <= 0:
            continue

        # 4. 选出不确定性最低的 top-k 样本
        _, top_k_relative_indices = torch.topk(discrepancy_class, k=num_to_select, largest=False)
        
        # 5. 获取这些被选中样本在整个数据集中的绝对索引
        selected_absolute_indices = candidate_indices[top_k_relative_indices]
        
        # 6. 更新掩码和标签
        selected_mask[selected_absolute_indices] = True
        final_selected_labels[selected_absolute_indices] = i # 直接赋予正确的类别标签 i

    # --- 日志记录 ---
    print('selected examples', selected_mask.sum())
    correct_selected_examples = (final_selected_labels[selected_mask] == clean_labels[selected_mask]).int().sum()
    print('Correct Selected examples:', correct_selected_examples)

    wandb.log({
        'Selected examples': selected_mask.sum().item(),
        'Correct selected examples': correct_selected_examples.item(),
    }, epoch)

    # --- 返回三个干净、可靠的结果 ---
    return selected_mask.float(), final_selected_labels, final_knn_pseudo_labels
