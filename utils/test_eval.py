...
def evaluate(encoder, classifier, loader, device):
    ...
    for batch_data in loader:
        if isinstance(batch_data, (list, tuple)) and len(batch_data) == 2: images, labels = batch_data
        else: continue
        valid = labels >= 0; images, labels = images[valid], labels[valid]
...
def test_eval(args, model, device, test_loader, epoch=0):
    ...
    return (loss_per_epoch, acc_val_per_epoch,acc)